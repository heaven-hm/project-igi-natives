...
This is the branch's only Project I.G.I. 1 native catalog and export bundle, generated from the live retail `D:\IGI1\igi.exe` analysis on `feature/natives-discovery`. `IGINatives.json` is the single source catalog for all 284 address-compatible native symbols, including the newly analyzed Qtask dispatch, camera-reset, and MagicObj helpers.
...
Heaven completed 80 native discoveries and labels as human reverse-engineering work. The remaining entries were AI-assisted using the live Ghidra Headless MCP and r2 MCP (Radare2 MCP), with names retained from `igi.exe` strings whenever available and behavior-derived names used only where the Ghidra/r2 evidence supported them. The 11 new entries in this update are recorded in the audit manifest with Ghidra/r2 CFG metrics; all accepted entries meet the strict static validation gate used for this pass.
